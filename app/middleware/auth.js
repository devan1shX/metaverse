...
function authenticateToken(req, res, next) {
  ...
  // If no token is provided, create a dummy user for development and bypass login.
  if (!token) {
    logger.info(
      "No token provided. Creating dummy user for development access."
    );
    req.user = {
      user_id: "dev-user-01",
      email: "developer@example.com",
      username: "DevUser",
      role: "admin", // Grant admin role for full access
    };
...
function requireParticipant(req, res, next) {
  if (!req.user) {
    logger.error("Participant check failed: No user in request", {
      path: req.path,
    });
    return res.status(401).json({
      message: "Authentication required.",
